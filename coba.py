...
login.configure(background='#1E1E2E')
...
def buka_form_member():
    ...
    def buka_kartu():
        ...
        diskon = {"BRONZE": "5%", "SILVER": "10%", "GOLD": "15%", "PLATINUM": "20%"}[level]
        ...
        ttk.Label(frame, text=f"Favorit: {snack}", font=('calibri', 12)).pack()
        ttk.Label(frame, text=f"Level  : {level}", font=('calibri', 12, 'bold')).pack(pady=5)
        ttk.Label(frame, text=f"Diskon : {diskon}", font=('calibri', 12, 'bold')).pack()
    ...
    ttk.Label(frame_input, text="Snack Favorit:", foreground="white", background="#1E1E2E").pack(anchor="w", pady=(10,0))
    ttk.Entry(frame_input, textvariable=SNACK).pack(fill='x')
